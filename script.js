document.addEventListener("DOMContentLoaded", () => {
  ...
  timelineSections.forEach((timelineSection) => {
    const timelineProgress =
      timelineSection.querySelector(".timeline-progress");
      ...
      fetch("/api/submit", {
        ...
        .then(async (response) => {
          ...
          if (response.status == 200) {
            btn.innerHTML =
              '<span>Message Delivered!</span> <i class="fa-solid fa-check"></i>';
            ...
            btn.innerHTML =
              '<span>Error, Please Retry!</span> <i class="fa-solid fa-triangle-exclamation"></i>';
            btn.style.background =
        ...
        .catch((error) => {
          btn.innerHTML =
            '<span>Error!</span> <i class="fa-solid fa-triangle-exclamation"></i>';
          btn.style.background =
            "linear-gradient(90deg, #ef4444, #dc2626, #ef4444)";
          if (window.soundSystem) window.soundSystem.playError();
        })
  ...
  if (filterBtns.length && skillCards.length) {
    filterBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const visibleCards = Array.from(skillCards).filter(
